...
<XafDefaultProperty("BlacklistNo")> _
Public Class BlacklistDetails
    Inherits BaseObject
    Private fBlacklistNo As Integer
    Private fMemberNo, fReason As String
    Public Sub New(ByVal session As Session)
        MyBase.New(session)
    End Sub
    <Appearance("DisableBlacklistNo", Enabled:=False)> _
    <RuleRequiredField()> _
    Public Property BlacklistNo() As Integer
        Get
            Return fBlacklistNo
        End Get
        Set(ByVal value As Integer)
            SetPropertyValue("BlacklistNo", fBlacklistNo, value)
        End Set
    End Property
    ...
    Protected Overrides Sub OnSaving()
        MyBase.OnSaving()
        Try
            If BlacklistNo = 0 And fReason Is Nothing = False Then
                Dim BlacklistNoNew As Integer = GenerateNumbers.GetBlacklistNo
                BlacklistNo = BlacklistNoNew
            End If
